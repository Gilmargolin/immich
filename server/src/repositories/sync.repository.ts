...
class AssetEditSync extends BaseSync {
  ...
  @GenerateSql({ params: [dummyQueryOptions], stream: true })
  getUpserts(options: SyncQueryOptions) {
    // Fork-custom filter: exclude rows whose `action` is one of this fork's
    // additions (Adjust, LensCorrection) from the mobile sync stream. The
    // upstream iOS / Android openapi-generated enum only knows crop / rotate /
    // mirror, so a row with an unrecognized action in the SyncAssetEditV1
    // payload makes mobile's JSON deserializer throw and the entire background
    // backup aborts before any upload POST (symptom: "cannot process backup"
    // on iPhone, manual uploads still work). The web client reads edits via
    // other endpoints and is unaffected.
    return this.upsertQuery('asset_edit', options)
      ...
      .where('asset.ownerId', '=', options.userId)
      .where('asset_edit.action', 'not in', [AssetEditAction.Adjust, AssetEditAction.LensCorrection])
      .stream();
  }
}
